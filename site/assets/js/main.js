...
function populateHeadlineRoster() {
  if (!DATA || !DATA.eventStudy || !DATA.eventStudy.headline_rosters) return;
  const rosters = DATA.eventStudy.headline_rosters;
  const treasuryCore = rosters.treasury_estimable_core || {};
  const flagship = rosters.flagship_clustered || {};
  const dropped = rosters.dropped_from_flagship || [];

  setText('headline-core-count', treasuryCore.entities || 0);
  setText('headline-flagship-count', flagship.entities || 0);
  setText('headline-core-roster', (treasuryCore.banks || []).join('; '));
  setText('headline-flagship-roster', (flagship.banks || []).join('; '));
  setText('headline-flagship-dropped', dropped.join('; '));
  setText('headline-roster-artifact', rosters.audit_artifact || '');
}
