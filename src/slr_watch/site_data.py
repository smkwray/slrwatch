...
def _clean_bank_names(values: pd.Series) -> list[str]:
    cleaned = (
    ...
    return [str(value) for value in cleaned]
...
def _load_event_study_payload(reports_root: Path) -> dict[str, object]:
    ...
    primary_pretrend = _read_csv(reports_root / "event_2020" / "pretrend_checks.csv")
    clustered_pretrend = _read_csv(reports_root / "event_2020" / "flagship_per_parent_clustered" / "pretrend_checks.csv")
    placebo_grid = _read_csv(reports_root / "event_2020" / "flagship_per_parent_clustered" / "placebo_fake_date.csv")
    leave_one_parent_out = _read_csv(reports_root / "event_2020" / "flagship_per_parent_clustered" / "leave_one_parent_out.csv")
    treatment_roster = _read_csv(reports_root / "event_2020" / "treatment_roster.csv")
            ...
            "primary": [
                {
                    ...
                    "p": float(row["pretrend_joint_pvalue"]),
                    "max_abs_pre_coef": float(row["max_abs_pre_coef"]),
                }
                for _, row in primary_pretrend[
                    primary_pretrend["outcome"].isin(["ust_inventory_fv_scaled"])
                    & primary_pretrend["treatment"].isin(["low_headroom_treated", "covered_bank_treated"])
                ].iterrows()
            ],
            "flagship_clustered": [
                {
                ...
                for _, row in clustered_pretrend[
                    clustered_pretrend["outcome"].isin(["ust_inventory_fv_scaled"])
                    & clustered_pretrend["treatment"].isin(["low_headroom_treated", "covered_bank_treated"])
                ].iterrows()
            ],
        },
        "placebo_grid": [
            {
                "label": str(row["placebo_grid_label"]),
                "treatment": str(row["treatment"]),
                "outcome": str(row["outcome"]),
                "coef": float(row["coef"]),
                "p": float(row["pvalue"]),
            }
            for _, row in placebo_grid[
                placebo_grid["outcome"].isin(["ust_inventory_fv_scaled"])
                & placebo_grid["treatment"].isin(["low_headroom_treated", "covered_bank_treated"])
            ].iterrows()
        ],
        "leave_one_parent_out": [
            {
                ...
                "outcome": str(outcome),
                "omitted_parents": int(subset["omitted_cluster"].nunique()),
                "coef_min": float(subset["coef"].min()),
                "coef_max": float(subset["coef"].max()),
                "p_min": float(subset["pvalue"].min()),
                "p_max": float(subset["pvalue"].max()),
            }
            for (treatment, outcome), subset in leave_one_parent_out.groupby(["treatment", "outcome"], dropna=False)
        ],
    }

    treasury_core_roster = _clean_bank_names(
        primary_prepared.loc[
            primary_prepared["covered_bank_treated"].notna(),
            "entity_name",
        ]
    )
    flagship_roster = _clean_bank_names(flagship_prepared["entity_name"])
    flagship_dropped = sorted(set(treasury_core_roster) - set(flagship_roster))
    ...
    return {
        ...
        "headline_rosters": {
            "treasury_estimable_core": {
                "entities": len(treasury_core_roster),
                "banks": treasury_core_roster,
            },
            "flagship_clustered": {
                "entities": len(flagship_roster),
                "banks": flagship_roster,
            },
            "dropped_from_flagship": flagship_dropped,
            "audit_artifact": "output/reports/event_2020/treatment_roster.csv",
            "treatment_map_artifact": "data/reference/insured_bank_treatment_map_2020.csv",
            "roster_version_count": int(treatment_roster["entity_id"].nunique()),
        },
    }
