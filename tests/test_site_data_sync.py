...
def test_public_copy_mentions_current_event_study_sample_ladder_counts() -> None:
    ...
    assert f"{int(samples['primary_core']['entities'])} balanced-coverage entities" in readme
...
def test_event_study_site_data_includes_headline_rosters() -> None:
    root = _repo_root()
    event_study = json.loads((root / "site" / "assets" / "data" / "event_study.json").read_text(encoding="utf-8"))
    rosters = event_study["headline_rosters"]

    assert rosters["treasury_estimable_core"]["entities"] == len(rosters["treasury_estimable_core"]["banks"])
    assert rosters["flagship_clustered"]["entities"] == len(rosters["flagship_clustered"]["banks"])
    assert rosters["audit_artifact"] == "output/reports/event_2020/treatment_roster.csv"
    assert "BANK OF AMERICA, NATIONAL ASSOCIATION" in rosters["treasury_estimable_core"]["banks"]
